...
import { fetchPostInterestCandidates } from '@/lib/discovery/postInterestPool';
...
export async function GET(request: NextRequest) {
  ...
  try {
    if (tags.length === 0) {
      // No interest signal — nothing to match against. Same empty-but-200
      // contract as every other discovery route.
      return NextResponse.json({ items: [], hasMore: false }, { status: 200 });
    }
    const { items, hasMore } = await fetchPostInterestCandidates(tags, limit, offset, username);
