...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${SIDECAR_URL}/new-snaps?since=${encodeURIComponent(since)}`, {
      signal: AbortSignal.timeout(3000),
    });
    if (!res.ok) return NextResponse.json({ count: 0, warming: false }, { status: 200 });
    const data = await res.json();
    // Response varies per `since` — never cache this route the way /active-users is cached.
