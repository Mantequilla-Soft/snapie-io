...
export async function POST(req: NextRequest) {
  try {
    ...
    const stored = await Challenge.findOneAndDelete({ nonce: challenge, username });
    if (!stored) {
      return NextResponse.json({ error: 'Invalid or expired challenge' }, { status: 401 });
    }

    const valid = await verifyHiveSignature(username, challenge, signature);
    if (!valid) {
      return NextResponse.json({ error: 'Signature verification failed' }, { status: 401 });
    }

    const token = signChatJWT(username);
